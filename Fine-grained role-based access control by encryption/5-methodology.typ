...
Lastly, OSWS, as mentioned, does not support #todo[ensure this is mentioned within system design] proper range request, and it is then apparent that S3 will perform even better for ranged request, where regardless of the range or full file request, S3 will perform close to the same regardless.#footnote[A solution to this is proposed within @sec:discussion.]
Thus, the choice was to set up the following e2e benchmarks.
Initially, it was intended to have five different-sized Parquet files, but there were some issues.#footnote[#todo[Add ref to result section mentioning issue]]
The five Parquet files will be auto-generated by a script.
These will have the following sizes:

- _Parquet Size Tiny:_\~100KB Parquet file with 50 columns and 1,000 rows.
...
#todo[Trøstrup argue why these sizes where chosen and see if the rest of the e2e info is correct. I should have updated everything accordingly above]

Python scripts will then run puts and gets against the following setups:

- Directly against Spaces
...
The metric for this will be latency, and for each file, the benchmark will be run for a total of 10 times, both cold and warm, allowing comparison of how slow or speed differences with and without caches.
