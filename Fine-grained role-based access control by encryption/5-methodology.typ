...
Lastly, OSWS, as mentioned, does not support proper range request, and it is then apparent that S3 will perform even better for ranged request, where regardless of the range or full file request, S3 will perform close to the same regardless.#footnote[A solution to this is proposed within @sec:discussion.]
...
DigitalOcean @digitalocean was chosen as hosting for both the benchmark driver through their Droplet virtual machine hosting, and S3-compatible storage through their Spaces Object Storage.
