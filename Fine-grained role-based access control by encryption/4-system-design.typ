...
For the prototype, this was simply chosen to be the user's first available role, but optimally should have been the role specified by the credential.
...
For the unauthorized columns, null/default data (dummy data) has to be copied over instead.

This choice, of Parquet Sharp, is one of the design choices made that later created more problems than it solved.

Even though the PME functions in Parquet Sharp are used, OSWS does not use PME in its intended way.
The goal was to, "Implement “client-side” encryption/decryption (storage client). The storage server must not see plaintext data, metadata or encryption keys."@parquet-modular-encryption-docs but that is broken, as the client considers OSWS to be the storage - as outward it is the storage but internally it is its own client.
This means PME was intended to be used to encrypt the Parquet file before the client inserts the data, but is instead done in OSWS, resulting in metadata that is modified and changed from how the client would expect it to be; e.g. metadata sizing.

=== Authentication
<sec:sys-design:authentication>

When clients communicate with Amazon S3 or S3-compatible API's like Cloudflare R2, an authentication scheme known as AWS Signature V4 @aws_signiture_version_4 is used. A pair of credentials known as the _client ID_ and _client secret_ is generated by the service, and both parties, client and service, store the information. When requests are made, the request is signed using the _client secret_ and the _client ID_ is appended as well. The service can find the credentials through the ID, and re-signs the request using the shared secret. The request is authenticated if the signatures match.
Since OSWS provides an S3-compatible API, authentication must follow this scheme as well. And, since OSWS re-writes the content, and re-routes the request, the original signature is no longer valid, so OSWS needs its own credentials aswell. Thus, OSWS both needs to verify the client's request, and correctly sign the new request to the object store.

=== AWS Signature V4 (S3 API)
<sec:sigv4>

To support AWS Signature V4, a custom `SigV4AuthenticationHandler` was created to parse the authorization header and extract the `AccessKeyId`, which is the ID used to identify the credential.
...
The backend has been made to be OIDC provider-agnostic, and the users can bring their own favourite provider.
...
The admins of the system can then define what the given clients can access in the RBAC frontend.
...
#new[
This also highlights that OSWS does not use range requests itself when querying S3, but provides the clients the option to use range requests, meaning that regardless of the range the clients query, OSWS fetches the whole Parquet file and then slices it for them.
]
...
But due to the limitations in Parquet Sharp, it was not possible, and the current solution is instead to create dummy data entries for the column.
