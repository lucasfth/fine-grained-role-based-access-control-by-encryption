...
The biggest change from the research paper is that OSWS does not support third-party query engines decrypting locally, but has to get the decrypted Parquet through OSWS.
As also mentioned later, "fully managed all-in-one cloud platforms", such as Snowflake or Databricks Unity Catalog, are not guaranteed to be supported as they would have to whitelist a URL on which OSWS runs, and have thus not been able to be tested; #delete[it is not certain that, given a whitelist, it would work.] // Virker som en gentagelse
